...
- ip-hash — hash function used to determine which server should be chosen for the next request (based on the client's IP address).
...
2. Configure Nginx for the routing part of the web application:
  ...
  * Configure the distribution of stats on the path /. In the root of the stats distribution place 2 files — index.html and image.png.
